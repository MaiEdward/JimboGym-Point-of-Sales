...
# Date: 3/15/2023
# All code is based on the CS 3409  flask starter app
...
def edit_class_participants(member_class_id):
    if request.method == "GET":
        ...
        query = "SELECT  Class_participants.member_class_id AS 'Class_Participant_ID', \
                ...
                JOIN Members ON Members.member_id = Class_participants.member_id \
                JOIN Classes ON Class_participants.class_id = Classes.class_id \
                WHERE Class_participants.member_class_id = %s;" % (member_class_id)
        cur = mysql.connection.cursor()
        cur.execute(query)
        data = cur.fetchall()
        ...
        #mySQL query to grab member name for dropdown
        query2 = "SELECT Members.member_id, CONCAT(Members.f_name, ' ',Members.l_name) AS 'member_name' FROM Members;"
        cur = mysql.connection.cursor()
        cur.execute(query2)
        member_data = cur.fetchall()
        ...
        #mySQL query to grab classes for dropdown
        query3 = "Select Classes.class_id, Classes.class_type FROM Classes;"
        cur = mysql.connection.cursor()
        cur.execute(query3)
        class_data = cur.fetchall()

        return render_template('edit_class_participant.j2', data=data, members=member_data, classes=class_data)
    
    if request.method == "POST":
        # fire off if user clicks the "Edit Person" button
        if request.form.get("Edit_Class_Participant"):
            # grab user form inputs
            member_class_id = request.form["Class_Participant_ID"]
            member_id = request.form["member_id"]
            class_id = request.form["class_id"]
